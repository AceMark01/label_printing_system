...
export default function HistoryPage() {
  ...
  const filteredHistory = history.filter(item => 
    ...
    (item.order_no || '').toString().toLowerCase().includes(searchQuery.toLowerCase())
  );
  ...
  return (
    <div className="space-y-12 animate-in fade-in duration-500 pb-12">
        ...
        <CardContent className="p-0">
          {loading ? (
              ...
              <table className="w-full text-left hidden md:table">
                ...
                <tbody className="divide-y divide-slate-50">
                  {filteredHistory.map((item) => (
                    <tr key={item.id} className="group hover:bg-white/80 transition-all duration-300">
            ...
            <div className="py-40 text-center px-8">
               ...
               <h3 className="text-2xl font-black text-slate-900 mb-2">No history archives</h3>
               <p className="text-slate-500 font-bold max-w-xs mx-auto">Start printing labels from the Orders page to populate your history log.</p>
            </div>
          )}
        </CardContent>
      </Card>
      ...
      <Dialog open={detailOpen} onOpenChange={setDetailOpen}>
        <DialogContent className="max-w-2xl w-[95vw] rounded-[2rem] md:rounded-3xl p-0 overflow-hidden border-none shadow-2xl">
          ...
          <div className="p-6 md:p-8 bg-white grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6 md:gap-y-8 max-h-[60vh] overflow-y-auto custom-scrollbar">
            ...
            <InfoBlock label="Transporter" value={selectedItem?.transporter_name} icon={<Printer className="w-4 h-4" />} />
          ...
          <div className="p-5 md:p-6 bg-slate-50 border-t border-slate-100 flex flex-col md:flex-row justify-end gap-3">
             <Button variant="ghost" onClick={() => setDetailOpen(false)} className="rounded-xl font-bold h-12 order-2 md:order-1">Dismiss</Button>
             <Button 
                onClick={() => {
                  const regenLabel = {
                    ...selectedItem,
                ...
                className="rounded-xl font-black bg-blue-600 hover:bg-blue-700 text-white px-8 h-12 order-1 md:order-2 shadow-lg shadow-blue-500/20"
             >
                Re-Generate Label
             </Button>
          </div>
